...
*/
// Note:
// You can try compile and run command line "mpirun -np 2 ./mpicell 50 40 126",
// which means the 50 bits, 40 operations in rule 126. If you only lack any of those 
// parameters, the code would assume you have 128 bits, 100 operations in rule 30. 
...
void DistributeLeftAndRightVals(int myID, int numprocs, int *mylocalcells, int localsize, int *leftval, int *rightval)
{
  ...
  // even: send and receive; odd: receive and send
  //  *leftval = ;
  //  *rightval = 0;

   int next =  ( myID + 1 ) % numprocs;
   int prev = myID == 0 ? numprocs -1 : myID-1;
  //  printf("%d,%d,%d,%d",sendleft,sendright,recvleft,recvright);


   if (myID %2 == 0){

     // rethink
      MPI_Send(&myfirst,1,MPI_INT,prev,0,MPI_COMM_WORLD);
      MPI_Send(&mylast,1,MPI_INT,next,0,MPI_COMM_WORLD);
      MPI_Recv(leftval,1,MPI_INT,prev,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      MPI_Recv(rightval,1,MPI_INT,next,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
   }
   else{
   ...
   for (curiters = 0; curiters < maxiters; curiters++)
     {
       ...
       //     printf("I am node %d, my local world is now:\n",id);
      //  printWorld(localcells,localsize,id);
      int * newcellworld =  malloc(WORLDSIZE*sizeof(double));
      MPI_Gather(localcells,localsize,MPI_INT,newcellworld,localsize,MPI_INT,0,MPI_COMM_WORLD);

       if (id == 0)
	  {
	    printf("results of gather on node 0\n");
      mycellworld = newcellworld;
